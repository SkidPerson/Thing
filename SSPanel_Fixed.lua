...
if gethsfuncs then
    ...
    -- PROTEÇÃO FINAL: Wrapper de inicialização segura
local SafeInit = {
    InitDeps = function(data)
        local ok, result = pcall(function() return initDeps(data) end)
        if not ok then
    ...
    InitAfterMain = function()
        local ok, result = pcall(function() return initAfterMain() end)
        if not ok then
    ...
    Main = function()
        ...
        if tick() - startTime > MAIN_TIMEOUT then
            warn("[SSPanel] Main excedeu timeout")
        end
