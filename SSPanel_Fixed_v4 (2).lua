...
local function main()
    ...
    Console.Init = function()
        ...
        local lastTextSizeUpdate = 0
        Console.TextSizeBox.TextBox:GetPropertyChangedSignal("Text"):Connect(function()
            local currentTime = tick()
            if currentTime - lastTextSizeUpdate < 0.05 then return end
            lastTextSizeUpdate = currentTime
        ...
        Console.CommandLine.ScrollingFrame.TextBox:GetPropertyChangedSignal("Text"):Connect(function()
            local currentTime = tick()
            local currentText = Console.CommandLine.ScrollingFrame.TextBox.Text

            -- Proteção contra eventos duplicados do executor
            if currentTime - lastTextUpdate < 0.05 then return end
            -- Proteção contra texto que não mudou
            if currentText == lastTextContent then return end

            lastTextUpdate = currentTime
            lastTextContent = currentText

            local oneliner = string.gsub(currentText, "
", "    ")
            if oneliner ~= currentText then
                Console.CommandLine.ScrollingFrame.TextBox.Text = oneliner
                lastTextContent = oneliner
            end

            Console.CommandLine.ScrollingFrame.Highlight.Text = SyntaxHighlightingModule.run(lastTextContent)
        end)
...
Main = (function()
    ...
    Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
        local gui = create({
            ...
            {11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text= "Developed By Koyo",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
            {12,"UIGradient",{Parent={11},Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,1,0),NumberSequenceKeypoint.new(1,1,0),}),}},
            {13,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Version",Parent={2},Position=UDim2.new(1,-110,1,-35),Size=UDim2.new(0,105,0,20),Text="V1.5.0",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
